...
- 印刷ページ（S-0307）は自動で開かない。詳細画面に戻ったあと、トレーナーが「登録のご案内を表示」ボタンで**別タブで**開く（発行後は状態が「登録待ち（期限内）」に変わるため、このボタンに置き換わる）
- **経緯**：過去に「発行後にそのまま印刷ページへ同タブ遷移」「詳細画面へ戻しつつ `window.open` で別タブを開く」の 2 案を試したが、前者は印刷ページから戻ったときにブラウザキャッシュで発行前の状態が表示される問題、後者はブラウザのポップアップブロックで、いずれも実用にならず、当初の形（詳細画面へ戻す＋完了メッセージ）に戻した。「登録のご案内を表示」ボタン名は、押した後の変化がトレーナーに伝わることを狙った（詳細は requirements.md 6-3-6 備考）
