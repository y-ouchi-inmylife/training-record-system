...
| 内部API | - | GET | `/api/training-records/{id}/available-media` | トレーニング記録に紐づけ可能なメディア一覧を取得する | auth | 管理者、一般 |
...
| training_date | date | ● | required, date | トレーニング日（過去・未来日付とも入力可） |
| training_time | time | | nullable, date_format:H:i | トレーニング時刻 |
...
##### S-1301 メディア登録（ファイルのアップロード）画面

###### GET /media-records/create
...
メディアの一覧表示・詳細表示・編集・削除を行う。アップロードは登録画面（S-1301）から内部API（署名付きURL発行→レコード作成）で行う。
...
| client_id | integer | ● | required, exists:clients,id | クライアントID |
| training_date | date | ● | required, date | トレーニング日 |
| training_time | time | | nullable, date_format:H:i | トレーニング時刻 |
| trainer1_id | integer | ● | required, exists:trainers,id | 担当1 |
| trainer2_id | integer | | nullable, exists:trainers,id, different:trainer1_id | 担当2（担当1と異なること） |
...
##### GET /api/training-records/{id}/available-media

**概要**: トレーニング記録に紐づけ可能な（まだ紐づいていない）メディアの一覧を取得する内部API。編集画面（S-0404）のメディア追加モーダル（S-0404-M01）で使用する。
...
**処理**:
- 対象トレーニング記録に**まだ紐づいていない**メディアのみを候補とする（紐づけ済みは除外する）
- 登録者で絞り込む（デフォルトはログイン中のトレーナー）
- 登録日時の新しい順に並べ、最大表示件数（24件）を超える場合はページネーションする

**レスポンス**（JSON）:
- 成功：`{ "data": [ <メディア>, ... ], "meta": { <ページネーション情報> } }`
- 該当トレーニング記録なし：エラーを返す（HTTP 404）
...
**概要**: メディアアップロード用の署名付きURL（presigned PUT URL）を発行する内部API。登録画面（S-1301）で登録ボタン押下後、実ファイルのアップロードに先立って呼び出す。
