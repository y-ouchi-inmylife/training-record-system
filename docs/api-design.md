...
- 印刷ページ（S-0307）は自動で開かない。詳細画面に戻ったあと、トレーナーが「マイページ登録案内を表示」ボタンで**別タブで**開く（発行後は状態が「登録待ち（期限内）」に変わるため、このボタンに置き換わる）
- **経緯**：過去に「発行後にそのまま印刷ページへ同タブ遷移」「詳細画面へ戻しつつ `window.open` で別タブを開く」の 2 案を試したが、前者は印刷ページから戻ったときにブラウザキャッシュで発行前の状態が表示される問題、後者はブラウザのポップアップブロックで、いずれも実用にならず、当初の形（詳細画面へ戻す＋完了メッセージ）に戻した。ボタン名は最終的に「マイページ登録案内を表示」に整え、押した後の変化がトレーナーに伝わるようにした（詳細は requirements.md 6-3-6 備考）
...
**レスポンス**:
- 成功：`redirect('/clients/{client}')` — **取消後は S-0305 詳細画面に戻る**。フラッシュ `success` に「マイページ登録案内を取り消しました。」を載せ、`layouts.app` の `session('success')` 表示で描画する。取消後は状態が「メールアドレスなし」に戻り、ボタン群は「マイページ登録案内を発行」だけが出る
...
- **期限切れは取り消せない**：期限切れの案内はもう使えず、取り消す意味がない。詳細画面のボタン群には「マイページ登録案内を発行」だけが出て、押下でその場で新規発行する（6-3-6 と同じ扱い）
