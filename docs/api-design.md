...
- **メールアドレス登録用 URL を無効にできるのはトレーナーだけ**（このエンドポイントの再発行操作）。お客様側の「入力し直す」操作では、メールアドレス登録用 URL のトークンは無効にならない（ログイン用リンクだけが差し替わる）
- URL の**個別削除**（`DELETE /clients/{client}/email-registration-tokens/{tokenId}`）は段階 4-1 の範囲外。発行ボタンを押せば常に有効な URL が 1 つある状態になるため、段階 4-1 では個別削除は不要。トレーナー側の状態表示・削除 UI は段階 4-2 でまとめて設計する
