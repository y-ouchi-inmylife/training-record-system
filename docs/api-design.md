...
- 成功（新規発行・再発行とも）：`redirect('/clients/{client}')` — **発行後は S-0305 詳細画面に戻る**。フラッシュに以下を載せる：
  - `success`：完了メッセージ（新規発行なら「メールアドレス登録用 URL を発行しました。」、再発行なら「メールアドレス登録用 URL を発行し直しました。」）。詳細画面が `layouts.app` の `session('success')` 表示で描画する
  - `open_print_url`：印刷ページ（S-0307）の URL。詳細画面のビュー側 JS がこれを読み取り、`window.open(url, '_blank', 'noopener')` で**別タブで印刷ページを開く**
- **この形にする理由**：発行後の詳細画面を発行後の状態で残したまま、渡すための紙面を別タブで並行して開くため。旧実装（印刷ページへ同タブ遷移）では、印刷ページから戻ったときにブラウザキャッシュで発行前の詳細画面が表示され、トレーナーが「発行できていない」と誤解する事象があった
- **ポップアップブロック時**：`window.open` が失敗しても詳細画面は正しく描画・更新されており、そこから「印刷ページを表示」ボタンで開き直せる。発行の処理はサーバ側で一度だけ実行される（POST-Redirect-GET）
...
**レスポンス**:
- 有効なトークンあり：view `clients.email-registration-token-print`。**印刷専用の新規レイアウト**（例：`layouts.print`）で描画する。`layouts.app` のナビゲーションバー・フッターは継承せず、レイアウト側で最初から出力しない。**ページ上部に「クライアント詳細に戻る」リンクを含める**（発行後は別タブで開かれるためタブを閉じても詳細画面に戻れるが、印刷ページを URL で直接開いた場合や、タブを閉じずに詳細画面へ移動したい場合の導線として残す。**画面表示のみで印刷時は非表示**。実装は Bootstrap の `d-print-none` を用いる）
- 有効なトークンなし：同じ view を返し、「発行済みの有効なメールアドレス登録用 URL がありません。クライアント詳細画面から発行してください」の案内を出す（**戻り導線はページ上部の共通リンクで済ませ、専用の CTA ボタンは置かない**。両状態で見せ方を揃える）。QR・URL・有効期限は出力しない
