...
- セッション有効期限（時間経過ログアウト）はトレーナー／クライアントで別の固定値とし、ミドルウェアがリクエストのサブドメイン（またはガード）を判定して、Cookie 名と有効期限をセットで動的に切り替える。無操作ログアウトはトレーナー側のみの機能で、クライアント側には適用しない（クライアント側は時間経過ログアウトのみで保護する）。
