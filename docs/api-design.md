...
クライアント・相談記録の参照・登録・更新・削除が成功したとき、操作者・操作種別・操作対象・IPアドレス・ユーザーエージェントを、カウンセラー操作履歴（access_logs）に記録する。記録される操作種別は、DB設計書の access_logs テーブル定義を参照。
...
| 相談記録管理 | S-0401 相談記録登録画面 | GET | `/counseling-records/create` | 相談記録登録画面を表示する | auth | 管理者、一般 |
| 相談記録管理 | S-0401 相談記録登録画面 | POST | `/counseling-records` | 相談記録を新規登録する（参加者含む） | auth | 管理者、一般 |
| 相談記録管理 | S-0402 相談記録一覧画面 | GET | `/counseling-records` | 相談記録一覧画面を表示する | auth | 管理者、一般 |
| 相談記録管理 | S-0403 相談記録詳細画面 | GET | `/counseling-records/{id}` | 相談記録詳細画面を表示する | auth | 管理者、一般 |
| 相談記録管理 | S-0403 相談記録詳細画面 | DELETE | `/counseling-records/{id}` | 相談記録を削除する | auth | 管理者、一般 |
| 相談記録管理 | S-0404 相談記録編集画面 | GET | `/counseling-records/{id}/edit` | 相談記録編集画面を表示する | auth | 管理者、一般 |
| 相談記録管理 | S-0404 相談記録編集画面 | PUT | `/counseling-records/{id}` | 相談記録を更新する（参加者含む） | auth | 管理者、一般 |
...
| カウンセラー管理 | S-0801 カウンセラー登録画面 | GET | `/counselors/create` | カウンセラー登録画面を表示する | auth | 管理者 |
| カウンセラー管理 | S-0801 カウンセラー登録画面 | POST | `/counselors` | カウンセラーを登録する | auth | 管理者 |
| カウンセラー管理 | S-0802 カウンセラー管理画面 | GET | `/counselors` | カウンセラー管理画面を表示する | auth | 管理者 |
| カウンセラー管理 | S-0802 カウンセラー管理画面 | PATCH | `/counselors/{id}/move-up` | カウンセラーの表示順を上に移動する | auth | 管理者 |
| カウンセラー管理 | S-0802 カウンセラー管理画面 | PATCH | `/counselors/{id}/move-down` | カウンセラーの表示順を下に移動する | auth | 管理者 |
| カウンセラー管理 | S-0802 カウンセラー管理画面 | PATCH | `/counselors/{id}/unlock` | アカウントロックを解除する | auth | 管理者 |
| カウンセラー管理 | S-0802 カウンセラー管理画面 | PATCH | `/counselors/{id}/toggle-active` | アカウントの有効/無効を切り替える | auth | 管理者 |
| カウンセラー管理 | S-0802 カウンセラー管理画面 | DELETE | `/counselors/{id}` | カウンセラーを削除する | auth | 管理者 |
| カウンセラー管理 | S-0803 カウンセラー編集画面 | GET | `/counselors/{id}/edit` | カウンセラー編集画面を表示する | auth | 管理者 |
| カウンセラー管理 | S-0803 カウンセラー編集画面 | PUT | `/counselors/{id}` | カウンセラーを更新する | auth | 管理者 |
| カウンセラー管理 | S-0804 パスワードリセット画面 | GET | `/counselors/{id}/reset-password` | パスワードリセット画面を表示する | auth | 管理者 |
| カウンセラー管理 | S-0804 パスワードリセット画面 | PUT | `/counselors/{id}/reset-password` | パスワードをリセットする | auth | 管理者 |
| カウンセラー管理 | S-0805 カウンセラー操作履歴画面 | GET | `/access-logs` | カウンセラー操作履歴画面を表示する | auth | 管理者 |
...
| マスタ管理 | S-0902 相談内容マスタ画面 | GET | `/master/consultation-types` | 相談内容マスタ管理画面を表示する | auth | 管理者 |
| マスタ管理 | S-0902 相談内容マスタ画面 | POST | `/master/consultation-types` | 相談内容の選択肢を追加する | auth | 管理者 |
| マスタ管理 | S-0902 相談内容マスタ画面 | PATCH | `/master/consultation-types/{id}/move-up` | 相談内容の表示順を上に移動する | auth | 管理者 |
| マスタ管理 | S-0902 相談内容マスタ画面 | PATCH | `/master/consultation-types/{id}/move-down` | 相談内容の表示順を下に移動する | auth | 管理者 |
| マスタ管理 | S-0902 相談内容マスタ画面 | PUT | `/master/consultation-types/{id}` | 相談内容の選択肢を更新する | auth | 管理者 |
| マスタ管理 | S-0902 相談内容マスタ画面 | DELETE | `/master/consultation-types/{id}` | 相談内容の選択肢を削除する | auth | 管理者 |
...
| 統計・集計 | S-1101 相談記録数推移画面 | GET | `/statistics/clients` | 相談記録数推移画面を表示する | auth | 管理者、一般 |
...
| 内部API | - | POST | `/api/counseling-records/auto-create` | 音声記録の要約から相談記録を作成する | auth | 管理者、一般 |
...
| 内部API | - | GET | `/api/counselors` | 担当カウンセラーの選択候補を取得する | auth | 管理者、一般 |

※ 認証列は実装のミドルウェア区分を示す。`public`＝認証不要（誰でもアクセス可）、`guest`＝未認証ユーザー向け（ログイン済みはホーム画面へリダイレクト）、`auth`＝要認証（ログイン済みのカウンセラー）。
※ 権限列は認証後のロール制限を示す。`-`＝認証不要のため対象外、`全員`＝ログイン済みの全カウンセラー（システム管理者を含む）、`管理者、一般`＝システム管理者を除く実務カウンセラー、`管理者`＝管理者のみ、`システム管理者`＝システム管理者のみ。
...
| family_relationship | string | ● | required, in:本人,母,父,配偶者,きょうだい,子,祖父母,その他 | |
| last_name | string | △ | nullable, string, max:50 | `family_relationship=本人` の場合は必須 |
| family_last_name | string | △ | nullable, string, max:50 | `family_relationship≠本人` の場合は必須 |
...
| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| initial_consultation_date | date | ● | required, date | 初回相談日 |
...
**リクエスト**:
クライアント情報の項目（S-0301 と同じ。ただし主担当カウンセラー・支援状態・連携機関は受け付けず、登録後にカウンセラー側で設定する）。

**処理**:
- トークンの有効性を再チェック（無効ならエラー画面）
- クライアントを登録（internal_idを採番。主担当カウンセラー・支援状態・連携機関はNULL）
...
| keyword | string | 氏名・かなの複合検索（本人・家族の姓名およびそのかな、計8項目を部分一致でOR検索） |
| support_status_id | integer | 支援状態ID |
| primary_counselor_id | integer | 主担当カウンセラーID |
| date_from | date | 最終相談日（開始日） |
| date_to | date | 最終相談日（終了日） |
...
**レスポンス**:
- view `clients.show`（クライアント情報と相談記録一覧。相談記録は新しい順）
...
**処理**:
- 相談記録が1件でも存在する場合は削除不可
- 物理削除（論理削除ではない）

**レスポンス**:
- 成功：`redirect('/clients')`
- 相談記録あり：`redirect('/clients/{id}')` ＋「このクライアントには相談記録が登録されているため削除できません。」
...
相談記録は必ずクライアント詳細画面（S-0305）から登録する。
...
| consultation_date | date | ● | required, date | 相談日（過去・未来日付とも入力可） |
| consultation_time | time | | nullable, date_format:H:i | 相談時刻 |
| is_intake | boolean | | boolean | インテークフラグ |
| is_followup | boolean | | boolean | フォローアップフラグ |
| consultation_type_id | integer | | nullable, exists:consultation_types,id | 相談内容マスタID |
| consultation_detail | string | | nullable, string, max:255 | 相談内容の詳細 |
...
| record_content | text | | nullable, string | 相談記録 |
| impression | text | | nullable, string | 所感 |
| phase_id | integer | | nullable, exists:phases,id | フェーズID |
| attendance | string | ● | required, in:参加,キャンセル（連絡あり）,キャンセル（連絡なし） | 参加状況 |
| consultation_format | string | ● | required, in:対面,ビデオ通話,電話,メール,同行,訪問,その他 | 参加形態 |
| consultation_format_detail | string | | nullable, string, max:255 | 参加形態の詳細 |
| participants | array | | nullable, array | 参加者リスト |
| participants[].participant_type | string | | nullable, in:本人,支援者,母,父,配偶者,きょうだい,子,祖父母,その他 | 参加者区分（クライアントの本人との関係と異なり「支援者」を含む） |
| participants[].participant_detail | string | | nullable, string, max:255 | 参加者詳細 |
...
| name | string | クライアントの氏名（本人・家族の姓名およびそのかな、計8項目を部分一致でOR検索） |
| date_from | date | 相談日（開始日） |
| date_to | date | 相談日（終了日） |
| counselor_id | integer | 担当カウンセラーID（担当1・担当2のいずれかに一致） |
| attendance | string | 参加状況 |
| consultation_format | string | 参加形態 |
| keyword | string | 記録内容・所感・相談内容詳細を対象としたキーワード検索（部分一致でOR検索） |
...
**処理**:
- 物理削除（論理削除ではない）。紐づく参加者も同時に削除される

**レスポンス**:
- 成功：その相談記録が紐づくクライアントの詳細画面へリダイレクト（`redirect('/clients/{client_id}')`）
- 権限なし：403エラー（「管理者のみ削除できます。」）
...
**処理**:
- 相談記録を更新し、紐づく参加者（複数可）を全削除後に再作成する
- 参加者だけを変更した場合も、相談記録の更新日時・更新者を更新する
...
録音して音声記録を作成することがこの画面の主機能。さらに、作成した音声記録に文字起こし・要約を行い、その要約から相談記録を作成することも可能。
...
4. POST /api/counseling-records/auto-create で要約から相談記録を作成
...
| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| counselor_id | string | | — | カウンセラーで絞り込む。未指定はログイン中のカウンセラー、`all` で全カウンセラー、カウンセラーIDの指定でそのカウンセラーの音声記録 |
...
**処理**:
- システム管理者を除くカウンセラーの一覧を表示する（各カウンセラーの主担当クライアント数、相談記録の担当件数を含む）
...
**概要**: カウンセラーの表示順を上に移動する。

**処理**:
- 一覧の表示順で、ひとつ上のカウンセラーと表示順を入れ替える
...
**概要**: カウンセラーの表示順を下に移動する。

**処理**:
- 一覧の表示順で、ひとつ下のカウンセラーと表示順を入れ替える
...
- 無効化されたカウンセラーはログインできなくなる。主担当の割り当ては残るが、主担当の付け替え候補には表示されない
...
**処理**:
- カウンセラーを削除する（物理削除）
- 次の場合は削除できない：システム管理者、自分自身、最後の管理者、主担当クライアントがいる、相談記録の担当者になっている
...
**概要**: カウンセラー操作履歴画面を表示する。

**クエリパラメータ**（すべて任意）:

| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| counselor_id | integer | | — | カウンセラーで絞り込む |
...
**処理**:
- 相談内容の一覧を表示順で表示する（各相談内容を参照している相談記録数を含む）
...
| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| name | string | ● | required, string, max:50, unique:consultation_types | 相談内容名 |
...
| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| name | string | ● | required, string, max:50, unique:consultation_types（自身を除く） | 相談内容名 |
...
**処理**:
- この相談内容を参照している相談記録がある場合は削除できない（物理削除）
...
**処理**:
- フェーズの一覧を表示順で表示する（各フェーズを参照している相談記録数を含む）
...
**処理**:
- このフェーズを参照している相談記録がある場合は削除できない（物理削除）
...
| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| counselor_id | string | | — | カウンセラーで絞り込む（`all` または カウンセラーID）。一般カウンセラー（staff）の場合は無視され、自分が担当する相談記録のみが集計される |
| view_type | string | | — | 表示タイプ（`fiscal_year`：年度別／`calendar_year`：年別）。既定は年度別 |
| period | integer | | — | 表示する期間（年度別なら年度、年別なら年） |

**処理**:
- 相談記録数の年度別または年別の推移、月別の推移、性別・年代別の内訳を集計する
...
**概要**: クライアントを検索する内部API。相談記録の作成・編集フォームなどで、クライアントを検索して選択（Select2）するために使用する。

**リクエスト**（クエリパラメータ）:
| パラメータ | 型 | 必須 | バリデーション | 説明 |
|-----------|-----|------|---------------|------|
| q | string | | — | 検索キーワード。管理番号、本人の氏名・かな、家族などの氏名・かなで部分一致検索する |
...
| client_id | integer | ● | required, exists:clients,id | クライアントID |
| consultation_date | date | ● | required, date | 相談日 |
| consultation_time | time | | nullable, date_format:H:i | 相談時刻 |
| consultation_format | string | ● | required, in:対面,ビデオ通話,電話,メール,同行,訪問,その他 | 参加形態 |
| counselor1_id | integer | ● | required, exists:counselors,id | 担当1 |
| counselor2_id | integer | | nullable, exists:counselors,id, different:counselor1_id | 担当2（担当1と異なること） |
| participants | array | | nullable, array | 参加者リスト |
| participants[].participant_type | string | | nullable, in:本人,支援者,母,父,配偶者,きょうだい,子,祖父母,その他 | 参加者区分 |
| participants[].participant_detail | string | | nullable, string, max:255 | 参加者詳細 |

**処理**:
- 音声記録の要約テキストを相談記録の記録内容に取り込み、相談記録と参加者を作成する
- 参加状況は「参加」で作成する

**レスポンス**（JSON）:
- 成功：`{ "success": true, "counseling_record_id": <作成した相談記録のID> }`
- 失敗：`{ "success": false }`
...
**概要**: 指定した音声記録の要約テキストを返す内部API。録音実行画面から相談記録の作成画面に遷移する際、対象の音声記録の要約を相談記録の記録内容に取り込むために使用する。
...
**概要**: 相談記録の作成・編集フォームで、音声記録の要約を取り込むために、候補となる音声記録の一覧を返す内部API。
...
**処理**:
- ログイン中のカウンセラーが所有し、指定したクライアントに紐づき、処理が完了して要約済みの音声記録を、作成日の降順で返す
...
**概要**: 担当カウンセラーの選択候補を返す内部API。録音実行画面で担当カウンセラーを選択するために使用する。

**処理**:
- システム管理者を除くカウンセラーを表示順に返す
