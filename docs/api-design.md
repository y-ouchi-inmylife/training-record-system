...
**レスポンス**:
- 有効なトークンあり：view `clients.email-registration-token-print`。**印刷専用の新規レイアウト**（例：`layouts.print`）で描画する。`layouts.app` のナビゲーションバー・フッターは継承せず、レイアウト側で最初から出力しない。**ページ上部に「クライアント詳細に戻る」リンクを含める**（本ページは詳細画面から別タブで開かれるためタブを閉じれば済むが、**URL を直接開いた場合**（ブックマーク・共有 URL 等）や、有効な URL がないときの案内から戻る導線として残す。**画面表示のみで印刷時は非表示**。実装は Bootstrap の `d-print-none` を用いる）
- 有効なトークンなし：同じ view を返し、「発行済みのマイページ登録案内がありません。クライアント詳細画面から発行してください」の案内を出す（**戻り導線はページ上部の共通リンクで済ませ、専用の CTA ボタンは置かない**。両状態で見せ方を揃える）。QR・URL・有効期限は出力しない
