...
**レスポンス**:
- 成功（新規発行・再発行とも）：`redirect('/clients/{client}/email-registration-tokens/print')` — **発行後は同じタブで S-0307 印刷ページを開く**（発行後の次の一手「お客様に渡す」作業をそのまま続けられるようにするため）
- 完了メッセージは**印刷ページには出さない**（印刷ページはお客様にも見せる／紙に出す想定であり、トレーナー向けの通知テキストが混ざらない方がよい）。フラッシュメッセージのやり取りは行わない
...
- **メールアドレス登録用 URL を無効にできるのはトレーナーだけ**（このエンドポイントの発行操作、または 6-3-7 メールアドレスの削除）。お客様側の「入力し直す」操作では、メールアドレス登録用 URL のトークンは無効にならない（ログイン用リンクだけが差し替わる）
...
**レスポンス**:
- 有効なトークンあり：view `clients.email-registration-token-print`。**印刷専用の新規レイアウト**（例：`layouts.print`）で描画する。`layouts.app` のナビゲーションバー・フッターは継承せず、レイアウト側で最初から出力しない。**ページ上部に「クライアント詳細に戻る」リンクを含める**（発行直後の遷移で開いたときの戻り導線。**画面表示のみで印刷時は非表示**。実装は Bootstrap の `d-print-none` を用いる）
- 有効なトークンなし：同じ view を返し、「発行済みの有効なメールアドレス登録用 URL がありません。クライアント詳細画面から発行してください」の案内と S-0305 へのリンクを出す（**有効なトークンありのときのページ上部リンクと見せ方を揃える**）。QR・URL・有効期限は出力しない
