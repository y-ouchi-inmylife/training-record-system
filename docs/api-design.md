...
| クライアント閲覧 | S-1406 クライアント登録情報設定画面 | GET | `/client-portal/settings` | 登録情報画面を表示する（基本情報フォーム＋メール・パスワード変更への入口） | auth:client | クライアント |
...
ログイン中のクライアントが、連絡先（基本情報）を変更する画面。あわせてメール変更（S-1409）・パスワード変更（S-1410）への入口を置く。

###### GET /client-portal/settings

**概要**: 登録情報画面を表示する（基本情報フォーム＋メール変更・パスワード変更への入口）。

**処理**:
- ログイン中のクライアントに紐づく現在の値（氏名・連絡先・現在のメールアドレス）を view に渡す

**レスポンス**:
- view `client.settings.index`（基本情報フォームと、メール変更・パスワード変更への入口カードを含む）
...
- 失敗：`back()` ＋ バリデーションエラーメッセージ（基本情報カード上部に表示）
...
**レスポンス**:
- 成功：`redirect('/client-portal/settings')` ＋「新しいメールアドレス宛に確認メールを送信しました。メールのリンクを開くとメールアドレスが切り替わります」（S-1406 の画面上部に表示）
...
**レスポンス**:
- 成功：`redirect('/client-portal/settings')` ＋「パスワードを変更しました」（S-1406 の画面上部に表示）
