...
| — | マスタ管理（ヘッダー） | — | — | |
| 8-4 | 支援状態 | S-0901 | 管理者 | |
| 8-5 | トレーニング内容 | S-0902 | 管理者 | |
| 8-6 | フェーズ | S-0903 | 管理者 | |
| — | （区切り） | — | — | |
| 8-7 | 自動ログアウト | S-1001 | 管理者 | |
...
| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| メディア一覧 | グリッド |  | この記録に紐づくメディアのサムネイルを表示順（昇順）で表示。サムネイルをクリックすると拡大表示・再生できる。1枚も紐づいていない場合は空表示 |
...
| メディア一覧 | グリッド |  | この記録に紐づくメディアのサムネイルを表示順（昇順）で表示。各サムネイル右上の×で紐づけを解除。ドラッグ＆ドロップで並べ替え可能。1枚も紐づいていない場合は空表示 |

メディアの追加・解除・並べ替えは画面上の仮状態とし、「更新」で他項目とまとめて保存する（「キャンセル」で破棄）。
...
| メディア一覧 | グリッド |  | 登録済みメディアのサムネイルを複数選択（チェック）できる。この記録に紐づけ済みのメディアは表示しない。件数が最大表示件数（24件）を超える場合はページネーションを表示。追加できるメディアがない場合は「追加できるメディアがありません」を表示 |
