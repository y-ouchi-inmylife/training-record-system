...
| 項目名 | 種類 | 説明 |
|--------|------|------|
| メールアドレス登録用 URL を発行 | ボタン | 1段目の操作ボタン群に常時表示。押下時、メールアドレス登録用 URL 発行モーダルを開く。モーダル内で発行済み URL の文字列表示・コピー・再発行・削除を扱う。詳しい状態別 UI（発行済み・未発行の切替、状態バッジ）は段階 4-2 で追加する |
...
| メールアドレス | 重複 | すでに他のクライアントで使われているアドレスの場合、「このメールアドレスは登録できません。担当トレーナーにご連絡ください」を表示して登録しない |
