...
| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| 閲覧状態 | 表示のみ |  | バッジ表示。メールアドレス未登録のときは「メールアドレス未登録」。閲覧が未解放のときは「未解放」。閲覧が解放済みでパスワード未設定のときは「解放（パスワード未設定）」。パスワード設定済みのときは「解放」。 |
| 閲覧を解放する | ボタン |  | 閲覧が未解放かつメールアドレス登録済みのときのみ表示。押下時、送信先メールアドレスを示した確認ダイアログを表示し、承認すると閲覧を有効化してパスワード設定用の招待メールを送信する |
| 解放を取り消す | ボタン |  | 閲覧が解放済み（is_viewable=true）のときのみ表示。押下時、確認ダイアログを表示し、承認すると閲覧を無効化し、設定済みパスワードと未使用の招待を破棄して解放前の状態に戻す |
...
│  ─────── トレーニング記録 ───────            │
│  ┌───────────────────────────────────────┐  │
│  │トレーニング日│担当1│担当2│トレーニング内容│← ヘッダー固定
│  │3/10 │佐藤  │田中  │基本トレーニング  ││
...
| トレーニング記録一覧 | テーブル |  | 「トレーニング日」が新しい順（降順）。すべて表示しきれない場合は縦スクロール表示。行クリックでクライアントトレーニング記録詳細画面（S-1404）へ遷移。データなし時は「トレーニング記録はありません」と表示 |
...
| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| ←ダッシュボード | リンク |  | クライアントダッシュボード画面（S-1402）へ遷移 |
...
| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| メディア一覧 | グリッド |  | この記録に紐づくメディアのサムネイルを表示順（昇順）で表示。サムネイルを選ぶと、写真は拡大表示、動画は再生できる。1枚も紐づいていない場合は空表示 |
