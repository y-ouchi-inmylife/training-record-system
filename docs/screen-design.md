...
| 閲覧状態 | バッジ | 氏名の右に表示。メールアドレス未登録のときは「メールアドレス未登録」、閲覧が未解放のときは「未解放」、解放済みでパスワード未設定のときは「解放中（パスワード未設定）」、パスワード設定済みのときは「解放中」 |
| 内部ID | 表示のみ | 閲覧状態バッジの右に「内部ID」ラベル（小さいグレー文字）と値を等幅フォントで表示 |

**3段目（属性 2 列。Bootstrap グリッドで `col-md-3` × 2。`md` 未満では縦積み）**
