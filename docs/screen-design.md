...
| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| 画面タイトル | 表示のみ |  | 「トレーニング記録詳細」。1段目の左に控えめな文字色で表示。同じヘッダー構造を持つクライアント詳細画面（S-0305）との識別を助けるために置く。S-0305 側にはタイトルを置かない（両画面にあるより、片方だけ異なる方が判別しやすいため） |
...
カード上部の 1 段目は、左端に画面タイトル「トレーニング記録詳細」を控えめな文字色で表示し、右端に `[« トレーニング記録一覧へ戻る] [編集] [削除]` の順でボタン群を並べる（ボタンの並び順は従来どおり）。同じヘッダー構造を持つクライアント詳細画面（S-0305）との識別を助けるために画面タイトルを置く。S-0305 側にはタイトルを置かない（両画面に同じ体裁の見出しがあるより、片方だけ異なる方が判別しやすいため）。
