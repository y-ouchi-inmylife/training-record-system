...
│  ■ 主担当クライアント一覧                                                            全N件     │
│  ┌───────┬──────────┬──────────┬──────────────┬──────────┬──────────┬──────────┬──────────┐  │
│  │内部ID │ 名前      │ 支援状態  │ 最終トレーニング日    │ 参加状況  │ 参加形態  │ 担当1  │ 担当2  │  │
│  ├───────┼──────────┼──────────┼──────────────┼──────────┼──────────┼──────────┼──────────┤  │
...
| 主担当クライアント一覧 | テーブル |  | ログインしているトレーナーが主担当、かつ、支援状態マスタで「ダッシュボード表示」がONになっている支援状態のクライアントを表示。最終トレーニング日が新しい順（降順）。最大10件表示。見出し右に「全N件」を表示し、10件超の場合は「すべて表示」リンク（クライアント一覧画面（S-0304）へ遷移）を表示。データなし時は「主担当のクライアントはありません」と表示 |
...
| 名前 | テキスト | 本人以外は家族名で表示 |
| 支援状態 | テキスト | バッジ表示 |
| 最終トレーニング日 | リンク | 最終トレーニングの日付。最終トレーニングのトレーニング記録詳細画面（S-0403）に遷移 |
| 参加状況 | テキスト | 最終トレーニングの参加状況。トレーニング記録がない場合は「—」と表示 |
| 参加形態 | テキスト | 最終トレーニングの参加形態。トレーニング記録がない場合は「—」と表示 |
...
| メモ | テキスト |  | 例：「山田太郎様 初回トレーニング予定 4/10 10:00」 |
| URL発行 | ボタン |  | 入力内容でクライアント登録用のURLを発行。「初回日」はトレーナーが指定した値がクライアント登録（URL発行）画面（S-0303）に引き継ぐ |
...
- 本画面はクライアント自身が入力する画面で、支援管理の項目は登録できない。トレーナーが入力するクライアント登録画面（S-0301）とはこの点が異なる。
...
│  25件のクライアント                   [新規登録]  │
│  ┌──┬────┬────┬──────┬──┬──┬────┬────┬──────┬────┐  │
│  │ID│名前│かな│本人関係│年齢│性別│主担当│支援状態│最終トレーニング日│フェーズ│  │
│  ├──┼────┼────┼──────┼──┼──┼────┼────┼──────┼────┤  │
...
| 最終トレーニング日（開始） | 日付 |  | 期間指定の開始日。最終日が未指定の場合は指定日以降のクライアントを表示 |
| 最終トレーニング日（終了） | 日付 |  | 期間指定の終了日。開始日が未指定の場合は指定日以前のクライアントを表示 |
...
│  │トレーニング日│参加者│担当1│担当2│参加状況│参加形態│初回│トレーニング内容│フェーズ│← ヘッダー固定
│  │3/10 │本人 │佐藤  │田中  │参加  │対面  │  │就労トレーニング│就職支援││
│  │2/15 │本人 │田中  │     │参加  │電話  │  │生活トレーニング│個人支援││
│  │1/20 │本人 │佐藤  │     │参加  │対面  │●│心理トレーニング│個人支援│↕ スクロール
│  │...（5件以上で縦スクロール表示）                 ││
│  └────────────────────────────────────────────── ┘│
│                                                   │
│  ┌─ 支援管理 ──────────────────────────── [▼] ┐│
...
| トレーニング記録一覧 | テーブル |  | 「トレーニング日」「トレーニング時刻」が新しい順（降順）。すべて表示しきれない場合は縦スクロール表示。行クリックでトレーニング記録詳細画面（S-0403）へ遷移。データなし時は「トレーニング記録はありません」と表示 |

**トレーニング記録一覧（テーブル）**

| 列 | 種類 | 説明 |
|------|------|------|
| トレーニング日 | 表示のみ | 今日より未来の場合は青色で表示し、予約・予定のトレーニングと視覚的に区別 |
...
|--------|------|------|------|
| キャンセル | ボタン |  | 遷移元の画面に戻る |
| 登録 | ボタン |  | トレーニング記録を登録し、クライアント詳細画面（S-0305）へ遷移 |
...
|--------|------|------|------|
| クライアント | 表示のみ | ● | |
| トレーニング日 | 日付 | ● | デフォルトは本日。過去・未来日付とも入力可（予約や将来のトレーニング予定の登録に対応） |
| トレーニング時刻 | 時刻 |  | |
...
│  15件のトレーニング記録                                            │
│  ┌──┬──────┬────┬────┬─────┬─────┬────┬────┬──┬──────┬────┐│
│  │内部ID│名前      │トレーニング日│参加者│担当1│担当2│参加状況│参加形態│初回│トレーニング内容│フェーズ││
│  ├──┼──────┼────┼────┼─────┼─────┼────┼────┼──┼──────┼────┤│
│  │1 │山田太郎│3/10│本人 │佐藤  │田中  │参加  │対面  │  │就労トレーニング│就職支援││
│  │2 │鈴木花子│3/8 │本人 │田中  │     │ｷｬﾝｾﾙ│電話  │●│生活トレーニング│個人支援││
│  └──┴──────┴────┴────┴─────┴─────┴────┴────┴──┴──────┴────┘│
│                                                           │
│  [< 前へ]  1 2  [次へ >]                                  │
│                                                           │
...
| 内部ID | テキスト |  | 内部IDの部分一致で検索 |
| 名前 | テキスト |  | 姓名・かなの部分一致で検索 |
| トレーニング日（開始） | 日付 |  | 期間指定の開始日。終了日が未指定の場合は指定日以降のトレーニング記録を表示 |
...
|--------|------|------|------|
| キャンセル | ボタン |  | 遷移元の画面に戻る |
| 更新 | ボタン |  | トレーニング記録を更新し、クライアント詳細画面（S-0305）へ遷移 |
