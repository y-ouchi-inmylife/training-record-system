...
※ クライアントポータル（S-1401〜S-1404）の配色は `docs/client-portal-design-plan.md` §2 のトークンに従い、本表の適用外とする。
...
`[« クライアント一覧へ戻る] [閲覧を解放する / 閲覧の解放を取り消す] [編集] [削除]` の順で、カード上部に右寄せで配置する。
