...
│  ■ 主担当クライアント一覧                                                            全N件     │
│  ┌───────┬──────────┬──────────┬──────────────┬──────────┬──────────┐  │
│  │内部ID │ 名前      │ 支援状態  │ 最終トレーニング日    │ 担当1  │ 担当2  │  │
│  ├───────┼──────────┼──────────┼──────────────┼──────────┼──────────┤  │
│  │26001  │ 山田太郎  │ 支援中    │ 2026年1月15日│ 佐藤     │ 田中    │  │
│  │26007  │ 鈴木花子  │ 支援中    │ 2026年2月10日│ 佐藤     │ —       │  │
│  └───────┴──────────┴──────────┴──────────────┴──────────┴──────────┘  │
│                                                                                              │
└──────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
│  │トレーニング日│担当1│担当2│トレーニング内容│フェーズ│← ヘッダー固定
│  │3/10 │佐藤  │田中  │就労トレーニング│就職支援││
│  │2/15 │田中  │     │生活トレーニング│個人支援││
...
│  15件のトレーニング記録                                            │
│  ┌──┬──────┬────┬─────┬─────┬──────┬────┐│
│  │内部ID│名前      │トレーニング日│担当1│担当2│トレーニング内容│フェーズ││
│  ├──┼──────┼────┼─────┼─────┼──────┼────┤│
│  │1 │山田太郎│3/10│佐藤  │田中  │就労トレーニング│就職支援││
...
| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| 登録 | ボタン |  | クリック後、ファイルをアップロードし、完了後にメディアを作成してメディア一覧画面（S-1302）に遷移。アップロード中は無効化 |
| キャンセル | ボタン |  | メディア一覧画面（S-1302）に戻る。アップロード中は無効化 |
| クライアント | プルダウン | ● | クライアント一覧から選択 |
| ファイル | ファイル選択 | ● | |
| 進捗バー | プログレスバー |  | アップロード中のみ表示。アップロードの進捗率（%）を表示 |
...
| 状態 | 説明 |
|------|------|
| 入力中 | クライアント・ファイルを選択できる。登録ボタン押下可能 |
| アップロード中 | 登録ボタン押下後。フォーム全体を入力不可にし、進捗バーと「アップロード中…画面を閉じないでください」を表示。登録・キャンセルボタンは無効化 |
| 完了 | アップロードとメディア作成が成功。メディア一覧画面（S-1302）へ遷移 |
| エラー | アップロード失敗、または形式・サイズ超過。入力中に戻し、エラーメッセージを表示 |
...
|--------|------|------|------|
| 登録者 | プルダウン |  | 選択肢: 全員+ 登録者一覧（システム管理者を除く）。デフォルトは、ログイン中のトレーナー |
| メディア一覧 | グリッド |  | サムネイルクリックでメディア詳細モーダルを表示。件数が最大表示件数を超える場合はページネーション（前へ／ページ番号／次へ）を表示 |
