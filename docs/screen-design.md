...
本画面は3ステップのウィザード形式。各ステップの項目は以下の通り。

| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| プログレスバー | 表示のみ |  | 現在のステップ番号に応じて幅を更新 |
| 戻る | ボタン |  | 入力内容を保持したまま、前のステップへ戻る（バリデーションは実行しない）。ステップ2〜3で表示 |
| 次へ | ボタン |  | バリデーションチェックを実行し、エラーがあれば該当項目にエラー表示を行い、ステップは進めない。エラーがなければ次のステップへ進む。ステップ1〜2で表示 |
| 登録 | ボタン |  | 全ステップの入力内容を登録し、クライアント詳細画面（S-0305）へ遷移。ステップ3で表示 |
...
| 登録 | ボタン |  | 全ステップの入力内容を登録し、状態：完了へ遷移。ステップ2で表示 |
...
| 状態 | 説明 | 表示内容 |
|------|------|---------|
| 入力 | 有効なトークンでアクセスした時 | 2ステップの登録フォームを表示 |
