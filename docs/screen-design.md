...
│  ← クライアント一覧                                │
│                                                   │
│  クライアント詳細        [URL発行]  [編集]  [削除]  │
│                                                   │
│  ─── 初回情報入力URL (https://…/token/xxxx) ─── [URLをコピー] [QRコード] [削除] │
│                                                   │
│  ┌─────────────────────────────────────────────┐│
...
│  │3/10 │佐藤  │田中  │就労トレーニング│3││
│  │2/15 │田中  │     │生活トレーニング│1││
│  │1/20 │佐藤  │     │心理トレーニング│—│↕ スクロール
│  │...（5件以上で縦スクロール表示）                 ││
│  └────────────────────────────────────────────── ┘│
│                                                   │
│  ┌─ 連絡先 ──────────── 入力なし ── [▶] ┐│
...
有効な初回情報入力URL（未使用かつ期限内）を1件のみ表示する。
使用済み・期限切れのURLは表示しない。

セクション見出し「初回情報入力URL」の後に、発行済みの初回情報入力URL文字列を括弧付き・薄いグレーで併記する。カードヘッダーの幅に収まらない場合はURL末尾から省略表示し、マウスホバーで全体を表示する。見出しラベルとヘッダー右のボタンは常に完全表示する。

| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| URL発行 | ボタン |  | 画面右上、[編集] ボタンの左に表示。有効なURLが存在する場合は非表示。押下時、有効期限を指定するモーダル（S-0305-M01）を開く |
| 初回情報入力URL | カード |  | 有効なURLが存在する場合のみ表示。有効なURLが存在しない場合は、初回情報入力URLセクション自体を表示しない |
...
| URLをコピー | ボタン | セクション見出しの右に表示。クリップボードにURLをコピー |
| QRコード | ボタン | セクション見出しの右に表示。QRコード表示モーダル（S-0305-M02）を開く |
| 削除 | ボタン | セクション見出しの右に表示。URLを削除する。削除後は再発行できる |
