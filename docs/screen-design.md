...
上段（1行目）と下段（属性 4 列）を `border-top` で区切って 1 つのカード内に収める。
...
`[« クライアント一覧へ戻る] [URL発行 / URL発行済み・残りN日] [編集] [削除]` の順で配置する。
...
**下段（属性 4 列。Bootstrap グリッドで `col-md-3` × 4。`md` 未満では縦積み）**
...
各項目は「ラベル上（小さいグレー文字）・値下」のセルとし、Bootstrap グリッド `col-6 col-md-4`（モバイル 2 列 / `md` 以上で 3 列）で並べる。値が空でも行高が変わらないよう、値部には最小高さを持たせる。ヘッダーサマリー下段の属性表示と同じ見た目にする。
