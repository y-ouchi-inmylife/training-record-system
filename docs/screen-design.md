...
│  │ID│名前│かな│主担当│最終記録日│        │
│  ├──┼────┼────┼────┼──────┤        │
│  │1│山田太郎│やまだ│佐藤│2026/3/10│        │
│  │2│鈴木花子│すずき│田中│2026/2/15│        │
│  │...│    │    │    │        │        │
│  └──┴────┴────┴────┴──────┘        │
│                                                 │
│  [< 前へ]  1 2 3  [次へ >]                      │
...
│                                                                  │
│ ┌─ ヘッダーサマリー ─────────────────────────────────────────┐  │
│ │   [« クライアント一覧へ戻る] [メールアドレス登録用 URL を発行] [編集] [削除]              │
│ │                                                              │  │
...
- 最終記録日は表示しない
- 「メールアドレス登録用 URL を発行」ボタンは1段目の操作ボタン群に置く。押下でメールアドレス登録用 URL 発行モーダルを開き、発行済み URL の文字列表示・コピー・再発行を扱う
- クライアントの状態（メールアドレスなし／メールアドレス登録待ち／初回設定待ち／利用中）に応じた状態バッジの表示・詳細な UI は段階 4-2 で追加する
...
`[« クライアント一覧へ戻る] [メールアドレス登録用 URL を発行] [編集] [削除]` の順で、カード上部に右寄せで配置する。

| 項目名 | 種類 | 説明 |
|--------|------|------|
| メールアドレス登録用 URL を発行 | ボタン | 1段目の操作ボタン群に常時表示。押下時、メールアドレス登録用 URL 発行モーダルを開く。モーダル内で発行済み URL の文字列表示・コピー・再発行を扱う。**未発行時の発行と、既に有効な URL がある場合の再発行を同じ操作で扱う**（同じボタン）。再発行時は、それまでのメールアドレス登録用 URL と送信済みのログイン用リンクの両方が無効になる。詳しい状態別 UI（発行済み・未発行の切替、状態バッジ）と個別削除は段階 4-2 で追加する |
...
- クライアントの状態（メールアドレスなし／メールアドレス登録待ち／初回設定待ち／利用中）に応じた状態バッジは段階 4-2 で追加する
...
段階 4-1 では未定義。クライアントの状態（メールアドレスなし／メールアドレス登録待ち／初回設定待ち／利用中）に応じた状態バッジと操作ボタンの詳細は段階 4-2 で追加する。
...
- URL の個別削除は段階 4-1 の範囲外（段階 4-2 でトレーナー側 UI とともに扱う）
