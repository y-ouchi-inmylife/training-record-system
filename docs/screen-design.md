...
| 配色 | 意味 | 例 |
|------|------|-----|
| `bg-success` | 正常・完了 | 解放中、未使用（有効なURL） |
| `bg-warning text-dark` | 要対応・注意 | 解放中（パスワード未設定） |
| `bg-secondary` | 未着手・無効 | 未解放、メールアドレス未登録、使用済み |
...
※ クライアントポータル（S-1401〜S-1404 および client-intake 系画面）の配色は `docs/client-portal-design-plan.md` §2 のトークンに従い、本表の適用外とする。
...
| 送信 | ボタン |  | 全ステップの入力内容で更新し、状態：完了へ遷移。ステップ2で表示 |
...
- 本画面はクライアント自身が入力する画面で、内部ID・初回日・メールアドレス・主担当は変更できない。トレーナーが入力するクライアント編集画面（S-0306）とはこの点が異なる。
- メールアドレスはクライアント閲覧機能のログインIDを兼ねるため、本画面では表示・入力しない。
...
│  25件                   [新規登録]  │
│  ┌──┬────┬────┬──┬──┬────┬──────┐  │
│  │ID│名前│かな│年齢│性別│主担当│最終記録日│  │
│  ├──┼────┼────┼──┼──┼────┼──────┤  │
│  │1│山田太郎│やまだ│45│男│佐藤│2026/3/10│  │
│  │2│鈴木花子│すずき│38│女│田中│2026/2/15│  │
│  │...│    │    │  │  │    │        │  │
│  └──┴────┴────┴──┴──┴────┴──────┘  │
│                                                 │
│  [< 前へ]  1 2 3  [次へ >]                      │
...
│                                                                  │
│ ┌─ ヘッダーサマリー ─────────────────────────────────────────┐  │
│ │   [« クライアント一覧へ戻る] [閲覧の解放を取り消す] [URL発行済み・残り6日] [編集] [削除] │
│ │                                                              │  │
...
**レイアウト方針**
- ヘッダーサマリー・2カラム本体・基本情報カード・連絡先カード・最終更新の 5 ブロックで構成する
- 2カラム本体は Bootstrap グリッドで左 `col-lg-8`（トレーニング記録）／右 `col-lg-4`（予備カード）に分ける。`lg` 未満では縦積みとする
- 現在の年齢・最終記録日は表示しない
- 初回情報入力URLは、常時表示のカードは廃止し、ヘッダー右のボタン＋モーダル（S-0305-M01）に集約する
- 閲覧解放ボタンは1段目の操作ボタン群に置く。閲覧状態に応じて排他的に表示する：未解放なら「閲覧を解放する」、解放済み（`is_viewable=true`）なら「閲覧の解放を取り消す」。メールアドレスが未登録でもボタン自体は表示し、押下時にその旨のメッセージを返して解放は行わない（ワイヤーフレームは代表的な1状態＝解放中の例示）
...
`[« クライアント一覧へ戻る] [閲覧を解放する / 閲覧の解放を取り消す] [URL発行 / URL発行済み・残りN日] [編集] [削除]` の順で、カード上部に右寄せで配置する。

| 項目名 | 種類 | 説明 |
|--------|------|------|
| URL発行 | ボタン | 有効な初回情報入力URLが存在しない場合の表示。ラベル「URL発行」、配色 `btn-outline-primary`。押下で S-0305-M01（未発行状態）を開く |
| URL発行済み・残りN日 | ボタン | 有効な初回情報入力URLが存在する場合の表示。ラベル「URL発行済み・残り〈N〉日」、配色 `btn-primary`。押下で S-0305-M01（発行済み状態）を開く。N は「有効期限までの残り日数」を切り上げで表示 |
| 閲覧を解放する | ボタン | 1段目の操作ボタン群に表示。閲覧が未解放のとき表示する（メールアドレスの登録有無を問わない）。押下時、送信先メールアドレスを示した確認ダイアログを表示し、承認すると閲覧を有効化してパスワード設定用の招待メールを送信する。メールアドレスが未登録の場合は、その旨のメッセージを表示して解放は行わない |
| 閲覧の解放を取り消す | ボタン | 1段目の操作ボタン群に表示。閲覧が解放済み（is_viewable=true）のときのみ表示。押下時、確認ダイアログを表示し、承認すると閲覧を無効化し、設定済みパスワードと未使用の招待を破棄して解放前の状態に戻す |
...
| 初回日 | 表示のみ | `Y/m/d` 形式 |
| 主担当 | 表示のみ | 主担当トレーナー名 |
| 閲覧状態 | バッジ | メールアドレス未登録のときは「メールアドレス未登録」、閲覧が未解放のときは「未解放」、解放済みでパスワード未設定のときは「解放中（パスワード未設定）」、パスワード設定済みのときは「解放中」 |
...
参照頻度が低いためヘッダーサマリーから下部に移した項目（生年月日・性別）を配置する。カード形式で常時展開表示する。表示形式は連絡先セクションと同じく、各項目を「ラベル上（小さいグレー文字）・値下」のセルとし、Bootstrap グリッド `col-6 col-md-4`（モバイル 2 列 / `md` 以上で 3 列）で並べる。
...
| 電話番号1 | 表示のみ |  |  |
| 電話番号2 | 表示のみ |  |  |
| メールアドレス | 表示のみ |  | クライアント閲覧機能のログインIDを兼ねる |

##### モーダル

###### S-0305-M01 初回情報入力URL モーダル

未発行状態と発行済み状態を 1 つのモーダルで扱う。ヘッダー右の「URL発行」ボタン（未発行）／「URL発行済み・残りN日」ボタン（発行済み）の押下で、それぞれ対応する状態のモーダルを開く。

**未発行状態**

| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| 見出し | 表示のみ |  | 「〈氏名〉 さんに初回情報を入力してもらうための URL を発行します」 |
| 前回状態の注記 | 表示のみ |  | 直近のトークンが無効（使用済み・期限切れ）の場合のみ表示。「以前発行したURLは〈使用済み／期限切れ〉です」 |
| 有効期限 | ボタングループ | ● | 1日後 / 7日後 / 14日後 / 30日後。デフォルト: 7日後 |
| 期限プレビュー | 表示のみ |  | 選択された有効期限に応じて、実際の期限日時（例：「2026/03/17 23:59 まで有効」）を表示 |
| キャンセル | ボタン |  | モーダルを閉じる |
| 発行する | ボタン |  | 選択した有効期限でURLを発行し、S-0305 に戻る。戻り先ではモーダルは開いていない状態 |

**発行済み状態**

| 項目名 | 種類 | 必須 | 説明 |
|--------|------|------|------|
| 状態バッジ + 残り日数 | 表示のみ |  | `ClientIntakeToken` の `status` アクセサの値（「未使用」）と残り日数を並べて表示（例：「未使用・残り〈N〉日」）。`$activeIntakeToken` は定義上「未使用かつ期限内」のため、この画面では常に「未使用」となる。モーダル上部に表示 |
| QRコード画像 | 画像 |  | トークンのURLをQRコードとして表示。モーダル open 時にクライアントサイドで生成する（既存の qrcodejs を利用） |
| URL | テキスト |  | QRコード画像の下に URL 全文を表示 |
| コピー | ボタン |  | URL テキストの右に配置。押下でクリップボードに URL をコピー |
| 発行日時 | 表示のみ |  | `Y/m/d H:i` 形式 |
| 有効期限 | 表示のみ |  | `Y/m/d H:i` 形式 |
| 削除 | ボタン |  | モーダルフッタ左に配置。押下でモーダル本体・フッタをインラインの削除確認表示に切り替える |
| 閉じる | ボタン |  | モーダルフッタ右に配置。モーダルを閉じる |

**削除確認（発行済み状態内のインライン確認）**

「削除」押下時、モーダル本体・フッタを以下の内容に差し替える（別モーダルは開かない）。

| 項目名 | 種類 | 説明 |
|--------|------|------|
| 確認メッセージ | 表示のみ | 「この初回情報入力URLを削除しますか？削除後は再発行できます。」 |
| キャンセル | ボタン | 発行済み状態のモーダル本体・フッタに戻す |
| 削除する | ボタン | URLを削除し、S-0305 に戻る。戻り先のヘッダーボタンは「URL発行」（未発行状態）に切り替わる |

QRコードのスキャンで、クライアントはクライアント編集（URL発行）画面（S-0303）にアクセスできる。

##### バリデーションチェック

S-0305-M01（未発行状態）の「発行する」押下時に、`expires_in_days` が 1／7／14／30 のいずれかであることをサーバー側で検証する。

##### 状態一覧

| 状態 | 契機 | 表示内容 |
|------|------|----------|
| URL未発行 | 有効な初回情報入力URLが存在しない | ヘッダーボタンは「URL発行」（`btn-outline-primary`） |
| URL発行済み | 有効な初回情報入力URLが 1 件存在する | ヘッダーボタンは「URL発行済み・残り〈N〉日」（`btn-primary`） |
| 閲覧未解放 | is_viewable=false かつ email 登録あり | 閲覧状態バッジ「未解放」＋「閲覧を解放する」ボタン |
| 閲覧未解放（メール未登録） | is_viewable=false かつ email 未登録 | 閲覧状態バッジ「メールアドレス未登録」＋「閲覧を解放する」ボタン（押下するとメールアドレス未登録である旨のメッセージを表示し、解放は行わない） |
| 閲覧解放中（パスワード未設定） | is_viewable=true かつ password 未設定 | 閲覧状態バッジ「解放中（パスワード未設定）」＋「閲覧の解放を取り消す」ボタン |
| 閲覧解放中 | is_viewable=true かつ password 設定済み | 閲覧状態バッジ「解放中」＋「閲覧の解放を取り消す」ボタン |
...
│ │      │のべ│ｸﾗ  │   性別     │          年齢            │      |
│ │      │トレーニング│実人│────┬──┬──┬──┼──┬────┬────┬────┬──┬──┬──┤     │
│ │ 年度 │記録数│数  │ 男 │女│無│未│≦19│2029│3039│4049│..│70│？│     │
│ ├──────┼────┼────┼────┼──┼──┼──┼──┼────┼────┼────┼──┼──┼──┤     │
...
│ │          │のべ│ｸﾗ  │   性別      │          年齢            │  │
│ │          │トレーニング│実人│────┬──┬──┬──┼──┬────┬────┬────┬──┬──┬──┤  │
│ │   月     │記録数│数  │ 男 │女│無│未│≦19│2029│3039│4049│..│70│？│  │
│ ├──────────┼────┼────┼────┼──┼──┼──┼──┼────┼────┼────┼──┼──┼──┤  │
