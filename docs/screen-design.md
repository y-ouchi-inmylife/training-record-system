...
│                                                                  │
│ ┌─ ヘッダーサマリー ─────────────────────────────────────────┐  │
│ │   [« クライアント一覧へ戻る] [URL発行済み・残り6日] [編集] [削除] │
│ │                                                              │  │
│ │ 山田 太郎（やまだ たろう）  [解放中] [閲覧の解放を取り消す]   内部ID  0001 │  │
│ │ ──────────────────────────────────────────────────────────── │  │
│ │  初回日       │  主担当                                       │  │
│ │  2026/01/20   │  佐藤                                         │  │
│ └──────────────────────────────────────────────────────────────┘  │
...
- ワイヤーフレームの氏名行に描いた「閲覧の解放を取り消す」ボタンは代表的な1状態（解放中）の例示。閲覧状態に応じて排他的に切り替わる：未解放かつメール登録済みなら「閲覧を解放する」、解放済み（`is_viewable=true`）なら「閲覧の解放を取り消す」、メール未登録ならいずれも表示しない
...
- 1段目: 操作ボタン群（右寄せ）
- 2段目: 氏名・閲覧状態バッジ・閲覧解放ボタン・内部ID
- 3段目: 属性 2 列（`border-top` で区切る）

**1段目（操作ボタン群）**

`[« クライアント一覧へ戻る] [URL発行 / URL発行済み・残りN日] [編集] [削除]` の順で、カード上部に右寄せで配置する。
...
| 閲覧を解放する | ボタン | 閲覧状態バッジの操作。閲覧が未解放かつメールアドレス登録済みのときのみ、氏名行の閲覧状態バッジの近傍に小さく表示。押下時、送信先メールアドレスを示した確認ダイアログを表示し、承認すると閲覧を有効化してパスワード設定用の招待メールを送信する |
| 閲覧の解放を取り消す | ボタン | 閲覧状態バッジの操作。閲覧が解放済み（is_viewable=true）のときのみ、氏名行の閲覧状態バッジの近傍に小さく表示。押下時、確認ダイアログを表示し、承認すると閲覧を無効化し、設定済みパスワードと未使用の招待を破棄して解放前の状態に戻す |
...
| 住所 | 表示のみ |  | 「都道府県」「市区町村」「町名・番地」を区切りなしで連結し、「建物名・部屋番号」がある場合は半角スペースで区切って続ける（例：「東京都新宿区西新宿1-2-3 ○○ビル301」）。連結対象がいずれも未登録の場合は「—」。一部のみ登録の場合は登録されている項目のみを連結し、余分な区切り文字が残らないようにする |
| 電話番号1 | 表示のみ |  |  |
| 電話番号2 | 表示のみ |  |  |
...
| 閲覧未解放（メール未登録） | is_viewable=false かつ email 未登録 | 閲覧状態バッジ「メールアドレス未登録」（操作ボタンなし） |
