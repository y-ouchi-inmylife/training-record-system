...
| QR コード生成 | QR コード生成ライブラリ（npm、ビルドに含める。具体パッケージ名は段階 4-2 実装時に確定して追記） | メールアドレス登録用 URL の QR コード表示（S-0305 発行モーダル）と印刷用ページ（S-0307）で使用。**外部 CDN からは読み込まない**（面談中にネット接続が不安定でも QR を表示できるようにするため） |
