...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('media_records', function (Blueprint $table) {
            ...
            $table->string('thumbnail_path', 500)->nullable()->comment('サムネイルのオブジェクトストレージ上の保存パス（キー）。一覧表示に用いる');
            ...
            $table->string('thumbnail_status', 20)->default('pending')->comment("サムネイル生成の状態: 'pending'（生成待ち、全メディアの初期値）, 'processing', 'done', 'error'");
            $table->timestamps();
        ...
        DB::statement("ALTER TABLE media_records ADD CONSTRAINT media_records_thumbnail_status_check CHECK (thumbnail_status IN ('pending', 'processing', 'done', 'error'))");
    }
