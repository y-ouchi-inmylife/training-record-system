...
{
    /**
     * クライアントテーブル作成（7カテゴリー50項目を1テーブルに集約）
     */
    public function up(): void
    {
        Schema::create('clients', function (Blueprint $table) {
            ...
            // カテゴリー1: 基本情報（14項目）
            $table->date('initial_consultation_date')->nullable()->comment('初回相談日');
            ...
            $table->string('family_relationship', 20)->nullable()->comment('本人との関係');
            ...
            $table->string('disability_physical_grade', 10)->nullable()->comment('身体障害者手帳の級');
            $table->string('disability_mental', 5)->nullable()->comment('精神障害者保健福祉手帳');
            $table->string('disability_mental_grade', 10)->nullable()->comment('精神障害者保健福祉手帳の級');
            $table->string('disability_intellectual', 5)->nullable()->comment('療育手帳');
            $table->string('disability_intellectual_grade', 10)->nullable()->comment('療育手帳の級');
            ...
            // カテゴリー7: 支援管理（3項目）
            $table->foreignId('primary_counselor_id')->nullable()->constrained('counselors')->nullOnDelete();
        ...
        DB::statement("ALTER TABLE clients ADD CONSTRAINT clients_family_rel_check CHECK (family_relationship IS NULL OR family_relationship IN ('本人', '母', '父', '配偶者', 'きょうだい', '子', '祖父母', 'その他'))");
        ...
        DB::statement("ALTER TABLE clients ADD CONSTRAINT clients_employment_period_check CHECK (employment_period IS NULL OR employment_period IN ('有期雇用（3ヵ月未満）', '有期雇用（3～6ヵ月未満）', '有期雇用（6ヵ月～1年未満）', '有期雇用（1年以上）', '無期雇用'))");
        DB::statement("ALTER TABLE clients ADD CONSTRAINT clients_unemployment_period_check CHECK (unemployment_period IS NULL OR unemployment_period IN ('6カ月未満', '6カ月～1年', '1～3年', '3～5年', '5～10年', '10年以上'))");
