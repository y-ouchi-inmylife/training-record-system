...
<div class="container py-4 c-dashboard">
    ...
    <h1 class="c-greeting">{{ auth('client')->user()->full_name }} 様、こんにちは</h1>
    ...
    @if($sessions->isEmpty())
        {{-- 空状態（記録0件）— 設計書 §4-2。記録の種別にトレーニング以外
             ...
             （2026-09 変更。当初は「ここに届きます」として本文の動詞
             『届く』と連動させていたが、記録がある場合の eyebrow
             「最新の記録」と表記が入れ替わり直下の説明文と意味がかぶる
             ため撤回した。詳細は client-portal-design-plan.md §4-2）。
             本文は初めてログインした会員に「この場所に何が並ぶか」を
             伝える役割があるためそのまま維持する。 --}}
