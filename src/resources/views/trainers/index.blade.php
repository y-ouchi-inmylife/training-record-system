                        ...
                        <tr>
                            <td>
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex gap-1">
                                        {{-- 上へ移動（先頭は無効化） --}}
                                        <form method="POST" action="{{ route('trainers.move-up', $counselor) }}" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn btn-outline-secondary btn-sm" {{ $index === 0 ? 'disabled' : '' }}>↑</button>
                                        </form>
                                        {{-- 下へ移動（末尾は無効化） --}}
                                        <form method="POST" action="{{ route('trainers.move-down', $counselor) }}" class="d-inline">
                                            @csrf
                                            @method('PATCH')
                            ...
                            <td class="text-end">
                                ...
                                <div class="d-flex gap-1 justify-content-end flex-wrap">
                                    ...
                                    @if($counselor->id !== Auth::id())
                                        <a href="{{ route('trainers.edit', $counselor) }}" class="btn btn-outline-primary btn-sm">編集</a>
                                        <a href="{{ route('trainers.reset-password', $counselor) }}" class="btn btn-outline-secondary btn-sm">PWリセット</a>
                                        @if($counselor->is_locked)
                                            <form method="POST" action="{{ route('trainers.unlock', $counselor) }}"
                                                  class="d-inline"
                                        ...
                                        @endif
                                        <form method="POST" action="{{ route('trainers.toggle-active', $counselor) }}"
                                              class="d-inline"
                                        ...
                                        @endphp
                                        <form method="POST" action="{{ route('trainers.destroy', $counselor) }}"
                                              class="d-inline"
